...
namespace Ion.Storage
{
    ...
    public class DatabaseManager
    {
        ...
        public int databaseClients
        {
            get
            {
                return GetOpenConnectionCount();
            }
        }
        ...
        /// <summary>
        /// returns starvation number
        /// </summary>
        public int getStarvationNumber()
        {
            return mClientStarvationCounter;
        }
        /// <summary>
        ...
        private void MonitorClientsLoop()
        {
            while (true)
            {
                try
                {
                    lock (this)
                    {
                        ...
                        for (int i = 0; i < mClients.Length; i++)
                        {
                            ...
                            {
                                if (mClients[i].Inactivity >= 45) // Not used in the last 30 seconds
                                {
                                    mClients[i].Disconnect(); // Temporarily close connection

                                    //Holo.Out.WriteLine("Disconnected database client #" + mClients[i].Handle);
                                }
                            }
                        }
                    }
                ...
                {
                    Holo.Out.WriteError(ex.Message);
                }
            }
        }
        /// <summary>
        ...
        public void ReleaseClient(uint Handle)
        {
            if (mClients.Length >= (Handle - 1)) // Ensure client exists
            {
                ...
                //Holo.Out.WriteLine("Released client #" + Handle);
            }
        }
