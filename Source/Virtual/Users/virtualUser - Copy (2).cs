    ...
    public class virtualUser
    {
            ...
            else
            {
                ...
                {                
                        ...
                        #region creditupdates
                            ...
                            int gimmiecredits = new int();
                            int oldcredits = new int();
                            using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
                            {
                                gimmiecredits = dbClient.getInt("SELECT userid FROM users_logins WHERE userid = '" + userID + "' AND date = '" + today + "'");
                                oldcredits = dbClient.getInt("SELECT credits FROM users WHERE id = " + userID);
                            }
                            if (gimmiecredits == 0)
                            {
                                ...
                                using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
                                {
                                    ...
                                    dbClient.runQuery("UPDATE users SET credits = " + newcredits + " WHERE id = " + userID);
                                    userManager.sendData("BK" + "Daily credits have been sent!\r\rEcho Staff!");
                                }
                            }
                            #endregion
                        ...
                        case "@g": // Messenger - request user as friend
                            {
                                if (Messenger != null)
                                {
                                    if (isLimited("sendfriendrequest"))
                                    {
                        ...
                        case "@e": // Messenger - accept friendrequest(s)
                            {
                                if (Messenger != null)
                                {
                                    if (isLimited("acceptfriendrequest"))
                                    {
                                        sendData("BKUnable to process requested action as this account is limited.");
                                        return;
                                    }
                        ...
                        case "@f": // Messenger - decline friendrequests
                            {
                                if (Messenger != null)
                                {
                                    if (isLimited("declinefriendrequest"))
                                    {
                                        sendData("BKUnable to process requested action as this account is limited.");
                                        return;
                                    }
                        ...
                        case "A@": // Enter room - add this user to room
                            {
                                if (_ROOMACCESS_SECONDARY_OK && Room != null && roomUser == null)
                                {
                                    if (!sentSettings)
                                    {
                                        StringBuilder sb = new StringBuilder("BK :fr = Friend Requests\r :followme = Stalking friends\r :filter = Wordfilter.\r\r");
                                        ...
                                        if (frEnabled)
                                            sb.Append(stringManager.getString("fr_enabled") + "\r");
                                        else
                                            sb.Append(stringManager.getString("fr_disabled") + "\r");

                                        if (filterEnabled)
                                            sb.Append(stringManager.getString("filter_enabled"));
                                        else
                                            sb.Append(stringManager.getString("filter_disabled"));
                        ...
                        case "D":
                            {
                                ...
                                try
                                {
                                    virtualUser Target = userManager.getUser(dbClient.Stripslash(currentPacket.Substring(4)));
                                    if (Target._Rank > 1)
                                        return;
                                    dbClient.runQuery("INSERT INTO user_ignores(userid,targetid) VALUES ('" + userID + "','" + Target.userID + "')");
                        ...
                        case "EB":
                            {
                                ...
                                try
                                {
                                    ...
                                    if (Target._Rank > 1)
                                        return;
                                    dbClient.runQuery("DELETE FROM user_ignores WHERE userid = '" + userID + "' AND targetid = '" + Target.userID + "'");
        ...
        internal void refreshSettings(int followMe, int friendRequest, int chatFilter, bool MUS)
        {
            ...
            using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
            {
                ...
                if (!MUS)
                    dbClient.runQuery("UPDATE users SET follow_me = '" + followMe.ToString() + "', friend_request = '" + friendRequest.ToString() + "', filter_chat = '" + chatFilter.ToString() + "' WHERE id = '" + userID + "' LIMIT 1");
        ...
        private bool isSpeechCommand(string Text)
        {
            ...
            try // Try/catch, on error (eg, target user offline, parameters incorrect etc) then failure message will be sent
            {
                switch (args[0].ToLower())
                {
                    ...
                    case "commands": // commands list
                        {
                            ...
                            ":brb - Alerts all users your on BRB MODE and amount of minutes \r" +
                            ":back - Alerts a I'm Back alert \r" +
                            ":staff - Shows alert on how to contact staff\r" +
                            ":about - Shows a description of the server. \r" +
                            ":cleanhand - Cleans your hand. \r" +
                            ":fr - Enables/Disables friend requests \r" +
                            ":followme - Enables/Disables friend requests\r" +
                            ":filter - Enables/Disables cursing filter\r" +
                    ...
                    case "cmds": // commands list
                        {
                            ...
                            ":shutdown - Closes the emulator.\r" +
                            ":stats - Shows furniture and registered user status\r" +
                            ":truncate - Clears the cms_help (CFH) table\r" +
                            ":kill - Destroys your connection socket\r" +
                            ":pet_user - Transforum another user into a pet (WARNING: NOT FIXED COMPLETELY!)\r" +
                            ":pet - Transform into a pet (WARNING: NOT FIXED COMPLETELY!)\r" +
                            ...
                            ":vip - <name> COMMAND IS IN TESTING MODE! - Gives a user VIP, if they purchase.\r" +
                            ":name - Edits your name, but has to be turned back! - 75% done with it. \r" +
                            ":badge - <name><badgeid> Gives a user a badge. \r" +
                            ":limit - Limits a user from doing hotel activites (60% done) \r" +
                            ":teletome - Brings a user to a higher ranked staff \r" +
                            ":cleanroom - Use if necessary, cleans the whole room.\r" +
                            ":massbadge <badgeid> - ONLY FOR COMPETITONS! \r" +
                    ...
                    case "teleport": // Toggles the user's teleport ability on/off
                        {
                            ...
                            else
                            {
                                ...
                                sendData("BK" + "Teleport is now on.");
                            }
                            break;
                        }

                    case "telewarp": // Warps the virtual user to a certain X,Y coordinate
                        {
                            if (rankManager.containsRight(_Rank, "fuse_administrator_access", userID) == false)
                                return false;
                            else
                    ...
                    #endregion
                    #region TRUNCATE
                    case "truncate": // Truncates the tables we tell it to. lol
                        {
                            using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
                                if (rankManager.containsRight(_Rank, "fuse_administrator", userID) == false)
                                {
                                    dbClient.runQuery("TRUNCATE `cms_help`");
                                    sendData("BK" + "You have successfully cleared the cms_help table!");
                                }
                            break;
                        }
                    #endregion
                    #region Kill User
                    case "kill": // attempt to kill a user's socket
                        {
                            using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
                            if (rankManager.containsRight(_Rank, "fuse_kill", userID))
                            {
                                try
                                {
                                    {
                                        if (userManager.getUser(dbClient.Stripslash(args[1]))._Rank >= _Rank)
                                            return false;
                                        virtualUser Target = userManager.getUser(dbClient.Stripslash(args[1]));
                                        try { Target.sendData("@c" + args[2]); }
                                        catch { Target.sendData("@cSocket killed by staff."); }
                                        Target.Disconnect();
                                    }
                                }
                                catch { }
                    ...
                    case "shutdown": // Shutdown in minutes.
                        {
                            if (rankManager.containsRight(_Rank, "fuse_administrator_access", userID) == false)
                            {
                            int Seconds = int.Parse(args[1]);
                            Seconds = 15;
                                userManager.sendData("Dc" + "Hotel is shutting down in approximately 15 seconds, come back later!" + Encoding.encodeVL64(Seconds));
                                Thread.Sleep(1000);
                   ...
                   case "maintenance": // Enables Maintenance and notifies all connected users, Created by AresCJ, Modified by NitroHabbz
                        {
                            if (rankManager.containsRight(_Rank, "fuse_administrator_access", userID) == false)
                            {
                                using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
                                dbClient.runQuery("UPDATE cms_system SET site_closed = 1");
                                userManager.sendData("***MAINTENANCE WARNING:***<br><br>* Hello hotel users, we are going into a brief maintenance.<br>The maintenance may take up to 10-30 minutes, depending on the issues we find.<br>Please visit the hotel's forum, for online updates.<br><br>Thanks,<br>Hotel Management.");
                            }
                            break;
                        }
                    ...
                    case "maintenanceoff": // Disables Maintenance and notifies all connected users, Created by NitroHabbz
                        {
                            ...
                            {
                                using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
                                    dbClient.runQuery("UPDATE cms_system SET site_closed = 0");
                                userManager.sendData("***MAINTENANCE COMPLETE:***<br><br>* Hello hotel users, The Hotel Maintenance was complete and the Hotel is now reopen to the public<br><br>Thanks for your patience,<br>Hotel Management.");
                            }
                            break;
                        }
                    #endregion
                    ...
                    case "happyhour": // Puts catalog at a half price  
                        {
                            if (rankManager.containsRight(_Rank, "fuse_administrator_access", userID) == false)
                                ...
                                using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
                                {
                                    ...
                                    userManager.sendData("BK" + "Happy Hour has now begun, please reload the hotel.");
                                    Thread Refresher;
                    ...
                    case "name":
                        {
                            if (!rankManager.containsRight(_Rank, "fuse_administrator_access", userID))
                                return false;
                            else
                            {
                                if (!nameChanged)
                    ...
                    case "badge":
                        {
                            using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
                            if (!rankManager.containsRight(_Rank, "fuse_give_badge", userID))
                                return false;
                            else
                            {
                                try
                                {
                                    virtualUser Target = userManager.getUser(dbClient.Stripslash(args[1]));
                                    dbClient.runQuery("UPDATE users_badges SET iscurrent = '0' WHERE userid = '" + Target.userID + "' AND iscurrent = '1'");
                                    dbClient.runQuery("INSERT INTO users_badges (userid,badgeid,iscurrent) VALUES ('" + Target.userID + "', '" + dbClient.Stripslash(args[2]) + "', '1')");
                        ...
                        #endregion
                    #region Vip command
                    case "vip": // Changes rank to rank 3 ":vip John"
                        {
                            using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
                                if (rankManager.containsRight(_Rank, "fuse_administrator_access", userID) == false)
                                {
                                        virtualUser member = userManager.getUser(dbClient.Stripslash(args[1]));
                                        dbClient.runQuery("UPDATE users SET rank = '3' WHERE id = '" + member.userID + "'");
                                        member.refreshAppearance(false, false, true);
                        }
                            break;
                        }
                    #endregion
                    #region Furniture TID Finder
                    // Furniture TID

                    /*case "furnituretids": // Search furni TID <_<
                        {
                            using(DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
                            if(rankManager.containsRight(_Rank, "fuse_administrator_access", userID) == false)
                            {
                                int id;
                                id = dbClient.getInt("");
                                sendData("BK" + "The furniture for the furniture you're looking for is:<br>'" + id +"'");
                            }
                            break;
                        }*/
                    #endregion
                    #region Limit command
                    case "limit": // Limit an account (toggle)
                        {
                            using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
                            if (!rankManager.containsRight(_Rank, "fuse_limit_account", userID))
                                return false;
                            else
                            {
                                try
                                {
                                    if (userManager.userExists(dbClient.Stripslash(args[1])))
                                    {
                                        try
                                        {
                                            virtualUser Target = userManager.getUser(dbClient.Stripslash(args[1]));

                                            if (Target._isLimited == 0)
                                            {
                                                Target._isLimited = 1;
                                                Target.sendData("BKYour account has been limited by a staff member, most features will therefore be disabled.<br>You can attempt to resolve this by emailing paul.b@openhotel.co.uk.<br><br>The following restrictions are in effect:<br>- No access to Account Settings on the website<br>- No access to any rooms except either ones you own or Support Desk<br>- No access to create any rooms<br>- No access to modifying any rooms you own<br>- No access to trading other users<br>- No access to buying a gift for another user<br>- No access to the catalogue");
                                                sendData("BKThe specified account has now been limited.");

                                                dbClient.runQuery("UPDATE users SET limited = 1 WHERE name = '" + dbClient.Stripslash(args[1]) + "'");
                                            }
                                            else if (Target._isLimited == 1)
                                            {
                                                Target._isLimited = 0;
                                                Target.sendData("BKYour account is no longer in a limited state.");
                                                sendData("BKThe specified account has had its limited state removed.");

                                                dbClient.runQuery("UPDATE users SET limited = 0 WHERE name = '" + dbClient.Stripslash(args[1]) + "'");
                                            }
                                            else
                                            {
                                                sendData("BKUnable to change limited state of the specified target.");
                                            }
                                        }
                                        catch
                                        {
                                            string[] targetData = dbClient.runReadRow("SELECT limited FROM users WHERE name = '" + dbClient.Stripslash(args[1]) + "'");
                                            if (int.Parse(targetData[0]) == 0)
                                            {
                                                dbClient.runQuery("UPDATE users SET limited = 1 WHERE name = '" + dbClient.Stripslash(args[1]) + "'");
                                                sendData("BKThe specified account has now been limited.");
                                            }
                                            else if (int.Parse(targetData[0]) == 1)
                                            {
                                                dbClient.runQuery("UPDATE users SET limited = 0 WHERE name = '" + dbClient.Stripslash(args[1]) + "'");
                                                sendData("BKThe specified account has had its limited state removed.");
                                            }
                                    ...
                                    {
                                        sendData("BKERROR: User not found.");
                                    }
                                }
                                catch { }
                            }
                    ...
                    case "teletome": // Teleports user to the room the sender is in
                        {
                            ...
                            else
                            {
                                ...
                                virtualUser _User = userManager.getUser(dbClient.Stripslash(args[1]));
                                virtualUser Target = userManager.getUser(dbClient.Stripslash(args[1]));
                    ...
                    #endregion
                    #region FollowMe / FR / Filter Options
                    case "followme":
                        {
                            if (isLimited("login"))
                            {
                    ...
                    case "fr":
                        {
                            ...
                            {
                                if (frEnabled)
                                    refreshSettings(getFollowMe, 0, getChatFilter, false);
                                else
                                    refreshSettings(getFollowMe, 1, getChatFilter, false);
                            }
                            break;
                        }
                    ...
                    case "filter":
                        {
                            ...
                            {
                                if (filterEnabled)
                                    refreshSettings(getFollowMe, getFriendRequest, 0, false);
                                else
                                    refreshSettings(getFollowMe, getFriendRequest, 1, false);
                            }
                            break;
