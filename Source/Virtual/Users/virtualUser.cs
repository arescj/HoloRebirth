...
namespace Holo.Virtual.Users
{
    ...
    public class virtualUser
    {
        ...
        /// <summary>
        /// Determine whether or not users can be friend requested.
        /// </summary>
        public bool frEnabled;
        /// <summary>
        /// Determine whether or not the chat filter is enabled for this user.
        /// </summary>
        ...
        /// </summary>
        /// System Timers and how they are set properly.
        ...
        public virtualUser(int connectionID, Socket connectionSocket)
        {
            ...
            try
            {
                string banReason = userManager.getBanReason(connectionRemoteIP);
                if (banReason != "")
                {
        ...
        private void dataArrival(IAsyncResult iAr)
        {
            ...
            catch (Exception e) { Out.WriteDCError(e.ToString()); sendData("BK Hotel has received an error, logged into DC.err!"); Disconnect(10000); }
        }
        #endregion
        #endregion
        ...
        #region Packet processing - Preferences
        /// <summary>
        ...
        private void processPacket(string currentPacket)
        {
            ...
            else
            {
                ...
                switch (currentPacket.Substring(0, 2))
                {
                    ...
                    case "Dn":
                        {
                            sendData("FY" + "QA1Bouncing Ball level 1III2Bouncing Ball level 2IJI3Bouncing Ball level 3IKI4Bouncing Ball level 4IPAI5Bouncing Ball level 5IQAHFYQA1Bouncing Ball level 1III2Bouncing Ball level 2IJI3Bouncing Ball level 3IKI4Bouncing Ball level 4IPAI5Bouncing Ball level 5IQAH");
                            break;
                        }
                    case "Dc":
                        {
        ...
        private object Clone(System.Collections.Hashtable hashtable)
        {
            throw new NotImplementedException();
        }
        #endregion

        #region Update voids - Preferences
        /// <summary>
        /// Refreshes
        /// </summary>
        ...
        internal void refreshAppearance(bool Reload, bool refreshSettings, bool refreshRoom)
        {
            ...
            if (refreshSettings)
            {
                if (imitateEnabled)
                {
                    string imitateName = ("SELECT name FROM users WHERE id = '" + imitateID + "'");
                    sendData("@E" + connectionID + Convert.ToChar(2) + imitateName + Convert.ToChar(2) + _Figure + Convert.ToChar(2) + _Sex + Convert.ToChar(2) + _Mission + Convert.ToChar(2) + Convert.ToChar(2) + "PCch=s02/53,51,44" + Convert.ToChar(2) + "HI");
                }
                else
        ...
        internal void abortTrade()
        {
            if (Room != null && Room.containsUser(_tradePartnerRoomUID) && roomUser != null)
            {
                ...
                _tradePartnerRoomUID = -1;
                _tradeAccept = false;
                _tradeItems = new int[65];
                _tradeItemCount = 0;
                statusManager.removeStatus("trd");
                roomUser.Refresh();
        ...
        #region followme / friend request / filter settings
        /// <summary>
        /// Update the user's follow me and friend request settings.
        /// </summary>
        /// <param name="followMe">0/1 to represent Disabled/Enabled for Follow Me.</param>
        /// <param name="friendRequest">0/1 to represent Disabled/Enabled for Friend Requests.</param>
        /// <param name="chatFilter">0/1 to represent Disabled/Enabled for Chat Filter.</param>
        /// <param name="MUS">Determine if this function is accessed by the MUS server.</param>
        internal void refreshSettings(int followMe, int friendRequest, int chatFilter, bool MUS)
        {
            ...
            using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
            {
                ...
                if (!MUS)
                    dbClient.runQuery("UPDATE users SET follow_me = '" + followMe.ToString() + "' WHERE id = '" + userID + "' LIMIT 1");

                sendData(sb.ToString());
            }
        }
        internal int getFollowMe
        {
            get
            {
                if (followEnabled)
        ...
        private bool isSpeechCommand(string Text)
        {
            ...
            try // Try/catch, on error (eg, target user offline, parameters incorrect etc) then failure message will be sent
            {
                ...
                {
                    #region Hotel commands (Go to command labeling region to edit)
                    #region Public commands (Gotta find the labeling to edit commands!)
                    #region User commands
                    case "commands": // commands list
                        {
                            goto Label_userCommands;
                        }
                    ...
                    case "whosonline": // Generates a list of users connected
                        {
                            goto Label_userWhosonline;
                        }
                    #endregion

                    #region :version
                    case "info": // Display information about the emulator
                        {
                            goto Label_userVersion;
                        }
                    #endregion
                    ...
                    case "staff": // How to Contact Hotel Staff
                        {
                            goto Label_staffMessage;
                        }
                    #endregion

                    #endregion

                    #region Moderacy commands (Gotta find their labels to edit the commands!)
                    #region MOD commands
                    case "modcmds": // commands list
                        {
                            goto Label_modCmds;
                        }
                    #endregion
                    ...
                    case "cmds": // commands list
                        {
                            goto Label_adminCommands;
                        }
                    #endregion

                    #region Free Space
                    case "freespace": // Free up MySQL Space
                        {
                            goto Label_adminFreespace;
                        }
                    #endregion
                    ...
                    case "back": // Stops the user from being shown as brb
                        {
                            goto Label_adminBack;
                        }
                    #endregion

                    #region Stats
                    case "stats": // (e.g. Furniture, users)
                        {
                            goto Label_adminStats;
                        }
                    #endregion
                    ...
                    case "userinfo": // Generates a list of information about a certain virtual user
                        {
                            goto Label_adminUserinfo;
                        }
                    #endregion

                    #region Shutdown // Maintenance
                    case "offline": // Broadcoasts a message that the server will shutdown in xx minutes
                        {
                            goto Label_adminOffline;
                        }

                    case "maintenanceon": // Enables Maintenance and notifies all connected users, Created by AresCJ, Modified by NitroHabbz
                        {
                            goto Label_adminonMaintenance;
                        }

                    case "maintenanceoff": // Disables Maintenance and notifies all connected users, Created by NitroHabbz
                        {
                            goto Label_adminoffMaintenance;
                        }
                    #endregion

                    #region Mass commands
                    case "masscredits": // Mass Credits command
                        {
                            goto Label_adminMasscredits;
                        }

                    #endregion

                    #region Happy Hour Functions
                    case "happyhour": // Puts catalog at a half price  
                        {
                            goto Label_adminHH;
                        }

                    case "endhappyhour": // Puts catalog at a half price  
                        {
                            goto Label_adminendHH;
                        }
                    #endregion

                    #region Cleanroom
                    case "cleanroom": // remove furniture in this room
                        {
                            goto Label_adminCleanroom;
                        }
                    #endregion
                    ...
                    #region Command Labeling
                    #region Command Labeling and descriptions
                    Label_userCommands:
                        if (rankManager.containsRight(_Rank, "default", userID) == false)
                        {
                            return false;
                        }
                    sendData("BK" +
                    "User Commands\r" +
                    "\r" +
                    ":about - Displays server info.\r" +
                    ":im <user> - Instant message a FRIEND ONLY.\r" +
                    ":staff - Shows alert on how to contact staff\r" +
                    ":about - Shows a description of the server. \r" +
                    ":cleanhand - Cleans your hand. \r" +
                    ":followme - Enables/Disables friend requests\r" +
                    ":whosonline - Show all users online \r" +
                    ":events - Pops up the event window.\r");
                    break;

                Label_userAbout:
                    sendData("BK" + "Echo 26 Emulator \r" +
                             "REV 1.2 (Mar 9th, 2009)\r" +
                             "Welcome to Echo!\r" + "");
                break;
            ...
            Label_userCleanhand:
                using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
                {
                    ...
                    sendData("BK" + "" + _Username + " hand is now cleaned!");
                }
            refreshHand("update");
            break;

        Label_userWhosonline:
    ...
    Label_userVersion:
        sendData("BK" + "Server Information \r" +
            ...
            "\r" +
            "Emulator: Echo Hotel Emulator\r" +
            "Build: 11.1.0.0 (28/01/2009)\r" +
            "DCR Support:  r26, \r");
    break;

Label_staffMessage:
    sendData("BK" + "To get in contact with staff, please use the BLUE '?' on the bottom right corner of your client window.<br>That's called the CFH Help System, so use it for emergencies only.<br><br>Still can't get in contact with us?<br><br>Please e-mail AresCJ@gmail.com about your issues.");
break;
                        #endregion

                    #region Moderator Commands Labeling
...
sendData("BK" +
"Moderator Commands\r" +
"\r" +
":alert <user> <message> - Alert a specific user.\r" +
":roomalert <message> - Alerts the room as a WHOLE\r" +
":kick <user> <message> - Kicks a specific user.\r" +
":roomkick <message> - Kicks the WHOLE room.\r" +
":shutup <user> <message> - Mutes a specific user.\r" +
":unmute <user> - Unmutes a specific user.\r" +
":roomshutup <message> - Mutes the whole room/\r" +
":roomunmute - Mutes a room\r" +
":ban <user> <hours> <message> - Bans a user.\r" +
":superban <user> <hours> <message> - Bans a user with hour options - Can perm.\r" +
":ha <message> - Hotel Alert.\r" +
":ra <message> - Same rank alert.\r" +
":teleport <on/off> - Teleports to hard to reach spots.\r" +
":telewarp X Y - Warps to a specified spot.\r" +
":refresh   - Catalog, Fuse, Eco, and etc.\r" +
":whosonline - Displays all users that are online \r" +
":stats - Shows furniture and registered user status\r" +
":kill - Destroys your connection socket\r" +
":teletome - Brings a user to a higher ranked staff \r" +
":wheres <username> - Tells you where's the user \r" +
":userinfo <username> - Display information on a specific user.\r");
break;
...
else
{
    ...
    virtualUser Usertarget = userManager.getUser(lookup);
    ...
    sendData("BK" + "The user " + args[1] + " is in the room \r" +
    "Room name: " + roomname + " \r" +
    "Owned by: " + roomowner + " \r" +
    "");
}
break;
...
break;
                    ...
                    #region Special staff Commands Labeling
...
":refresh   - Catalog, Fuse, Eco, and etc.\r" +
":whosonline - Displays all users that are online \r" +
":offline - Closes the emulator.\r" +
":freespace - Cleans MySQL Tables. \r" +
":stats - Shows furniture and registered user status\r" +
":maintenance - Turns hotel into maintenance mode.\r" +
":maintenanceoff - Turns hotel back into stable motion.\r" +
":happyhour - Discounts the furniture for the users. (WARNING: DO NOT USE UNLESS CJ GIVES PERMISSION! NOT COMPLETE!!) \r" +
":endhappyhour - Puts furniture back into stable prices. (WARNING: DO NOT USE!!)\r" +
":name - Edits your name, but has to be turned back! - 75% done with it. \r" +
":cleanroom - Use if necessary, cleans the whole room.\r" +
":masscredits <howmuch>  - Sends xx amounts of credits to everyone.\r" +
":userinfo <username> - Display information on a specific user.\r");
break;
...
using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
    ...
    else
    {
        ...
        Room.sendData("DBO" + "\r");
        staffManager.addStaffMessage("pickup", userID, 0, "Picked up all furniture in room " + Room.roomID.ToString(), "");
    }
break;
...
using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
    ...
    else
    {
        int furniCount;
        int userCount;
        furniCount = dbClient.getInt("SELECT COUNT(*) FROM furniture");
        userCount = dbClient.getInt("SELECT COUNT(*) FROM users");
        sendData("BK" + "Your hotel has " + furniCount + " amounts of furni on the hotel and " + userCount + " user(s) registered on the hotel.<br>Have a good day.");
        break;
    }
...
else
{
    ...
    Eucalypt.Shutdown();
    // Closing emulator peacefully.
}
break;

Label_adminonMaintenance:
if (rankManager.containsRight(_Rank, "fuse_administrator_access", userID) == false)
    return false;
{
    using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
        dbClient.runQuery("UPDATE cms_system SET site_closed = '1' WHERE site_closed = '0'");
    userManager.sendData("BK" + "***MAINTENANCE WARNING:***<br><br>Hello hotel users, we are going into a brief maintenance.<br>The maintenance may take up to 10-30 minutes, depending on the issues we find.<br>Please visit the hotel's forum, for online updates.<br><br>Thanks,<br>Hotel Management.");
}
break;

Label_adminoffMaintenance:
if (rankManager.containsRight(_Rank, "fuse_administrator_access", userID) == false)
    return false;
{
    using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
        dbClient.runQuery("UPDATE cms_system SET site_closed = '0' WHERE site_closed = '1'");
    userManager.sendData("BK" + "***MAINTENANCE COMPLETE:***<br><br>Hello hotel users, The Hotel Maintenance was complete and the Hotel is now reopen to the public<br><br>Thanks for your patience,<br>Hotel Management.");
}
break;

Label_adminMasscredits:
using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
    if (rankManager.containsRight(_Rank, "fuse_administrator_access", userID) == false)
        sendData("BK" + "You don't have the rights to send credits!");
    else
    {
        int Credits = int.Parse(args[2]);
        dbClient.runQuery("UPDATE users SET credits = " + _Credits + " + " + Credits + "");
        sendData("BK" + "You've succesfully sent " + Credits + " credits to all users!");
        refreshAppearance(false, true, false);
        refreshValueables(true, true);
    }
break;
...
if (rankManager.containsRight(_Rank, "fuse_administrator_access", userID) == false)
    ...
    using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
    {
        dbClient.runQuery("UPDATE catalogue_items SET catalogue_cost = (catalogue_cost / 2.5)");
        dbClient.runQuery("UPDATE catalogue_items SET catalogue_cost = '1' WHERE catalogue_cost = '0'");
        dbClient.runQuery("UPDATE catalogue_items SET catalogue_cost = '5' WHERE tid = '274'");
        dbClient.runQuery("UPDATE catalogue_items SET catalogue_cost = '10' WHERE tid = '275'");
        dbClient.runQuery("UPDATE catalogue_items SET catalogue_cost = '20' WHERE tid = '276'");
        dbClient.runQuery("UPDATE catalogue_items SET catalogue_cost = '50' WHERE tid = '277'");
        dbClient.runQuery("UPDATE catalogue_items SET catalogue_cost = '10' WHERE tid = '278'");
        dbClient.runQuery("UPDATE catalogue_items SET catalogue_cost = '50' WHERE tid = '279'");
        dbClient.runQuery("UPDATE catalogue_items SET catalogue_cost = '100' WHERE tid = '280'");
        dbClient.runQuery("UPDATE catalogue_items SET catalogue_cost = '200' WHERE tid = '281'");
        dbClient.runQuery("UPDATE catalogue_items SET catalogue_cost = '500' WHERE tid = '282'");
        dbClient.runQuery("UPDATE users_badges SET badgeid = 'ACH_HappyHour1' WHERE userid = '" + userID + "'");
        userManager.sendData("BK" + "Echo Hotel's Happy Hour has began, please reload the hotel's client.");
        Thread Refresher;
...
if (rankManager.containsRight(_Rank, "fuse_administrator_access", userID) == false)
    ...
    using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
    {
        ...
        dbClient.runQuery("UPDATE catalogue_items SET catalogue_cost = '200' WHERE tid = '281'");
        dbClient.runQuery("UPDATE catalogue_items SET catalogue_cost = '500' WHERE tid = '282'");
        userManager.sendData("BK" + "Echo Hotel's Happy Hour has ended, please reload the hotel's client.");
        Thread Refresher;
        ThreadStart tStarter = null;
        tStarter = new ThreadStart(refresh_cat);
        Refresher = new Thread(tStarter);
...
Label_adminCleanroom:
using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
    if (rankManager.containsRight(_Rank, "fuse_administrator_access", userID) == false)
        return false;
    else
    {
        dbClient.runQuery("DELETE FROM furniture WHERE roomid = '" + Room.roomID + "'");
        Room.sendData("DBO" + "\r");
        staffManager.addStaffMessage("cleanroom", userID, 0, "Cleaned room " + Room.roomID.ToString(), "");
    }
break;
...
else
{
    if (!nameChanged)
        ...
        using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
            _Username = dbClient.getString("SELECT name FROM users WHERE id = '" + userID + "'");
...
Label_followMe:
if (followEnabled)
    refreshSettings(0, getFriendRequest, getChatFilter, false);
else
    refreshSettings(1, getFriendRequest, getChatFilter, false);
break;
...
Label_adminRemoveusers:
if (rankManager.containsRight(_Rank, "fuse_administrator_access", userID) == false)
    return false;
{
    using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
    {
        int timeCheck = dbClient.getInt("SELECT lastvisit FROM users WHERE id = " + userID + "");
        if (timeCheck < 16-07-2009)
        {
            dbClient.runQuery("DELETE FROM users WHERE id = " + userID + "");
            dbClient.runQuery("DELETE FROM messenger_friendships WHERE userid = '" + userID + "'");
            dbClient.runQuery("DELETE FROM furniture WHERE ownerid = '" + userID + "'");
            dbClient.runQuery("DELETE FROM rooms WHERE owner = '" + _Username + "'");
            dbClient.runQuery("DELETE FROM soundmachine_songs WHERE userid = '" + userID + "'");
            dbClient.runQuery("DELETE FROM system_chatlog WHERE username = '" + _Username + "'");
            dbClient.runQuery("DELETE FROM users_badges WHERE userid = '" + userID + "'");
            dbClient.runQuery("DELETE FROM users_club WHERE userid = '" + userID + "'");
            dbClient.runQuery("DELETE FROM furniture WHERE ownerid = '" + userID + "' AND roomid = '0'");
            dbClient.runQuery("DELETE FROM group_details WHERE ownerid = '" + userID + "'");
            dbClient.runQuery("DELETE FROM group_memberships WHERE userid = " + userID + "");
            dbClient.runQuery("DELETE FROM users_favourites WHERE userid = " + userID + "");
            dbClient.runQuery("DELETE FROM users_recycler WHERE userid = " + userID + "");
            sendData("BKscommand_success");
        }
        else
        {
            if (timeCheck > 16-07-2009)
            {
                sendData("BKError deleting the users, due to invalid userIDs or the users are active");
            }
        }
    }
}
...
Label_adminFreespace:
if (rankManager.containsRight(_Rank, "fuse_administrator_access", userID) == false)
    return false;
{
    using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
    {
        dbClient.runQuery("TRUNCATE cms_help");
        dbClient.runQuery("TRUNCATE cms_alerts");
        dbClient.runQuery("REPAIR TABLE users");
        dbClient.runQuery("REPAIR TABLE rooms");
        dbClient.runQuery("REPAIR TABLE room_categories");
        dbClient.runQuery("TRUNCATE cms_transactions");
        sendData("MySQL is now complete freeing up space");
    }
}
break;
                    #endregion
        ...
        private void dailyTimer(int coins)
        {
            using (DatabaseClient dbClient = Eucalypt.dbManager.GetClient())
            {
                coins = 50;
                dbClient.runQuery("UPDATE users SET credits = (credits + " + coins + ") WHERE id = " + userID + "");
